...
export const getBacklinksAction = adminActionClient
    ...
    try {
      ...
      for (const filter of filters) {
        ...
        switch (filter.id) {
          ...
          case 'account':
            ...
            break;
          case 'status':
            conditions.push(eq(directory.status, filter.value));
            break;
          case 'source':
            conditions.push(eq(directory.source, filter.value));
            break;
        }
      }
      ...
      const query = db
        .select({
          ...
          status: directory.status,
          source: directory.source,
          rejectedReason: directory.rejectedReason,
          createdAt: directory.createdAt,
        })
        .from(directory)
      ...
      } else if (sortId === 'domain') {
        // Sort by extracted domain from URL
        // Extract domain: remove http://https://, remove www., get first part before /
        orderByClauses.push(
          isDesc
            ? sql`LOWER(REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(${directory.url}, '^https?://', ''), '^www\\.', ''), '/.*$', '')) DESC NULLS LAST`
            : sql`LOWER(REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE(${directory.url}, '^https?://', ''), '^www\\.', ''), '/.*$', '')) ASC NULLS LAST`
        );
      } else {
