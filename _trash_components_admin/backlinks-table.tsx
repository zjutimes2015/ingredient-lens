...
import { BacklinksDetailViewer } from '@/components/admin/backlinks-detail-viewer';
...
import { extractDomain, getExternalUrl } from '@/lib/urls/urls';
import {
  type ColumnDef,
...
export function BacklinksTable({
  ...
  // Using admin translations for now, ideally we would have a separate namespace for backlinks
  const t = useTranslations('Dashboard.admin.users'); // Fallback or reuse
  ...
  const columns: ColumnDef<Backlink>[] = useMemo(
    () => [
      {
        id: 'name',
        accessorKey: 'name',
        ...
        cell: ({ row }) => {
          return (
            <div className="pl-4 w-[200px] min-w-[200px]">
              <BacklinksDetailViewer directory={row.original} />
            </div>
          );
        },
        enableSorting: true,
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          const currentStatus = (row.getValue('status') as string) || 'active';
          ...
          if (!onStatusUpdate) {
            // Fallback to badge display if no update handler provided
            return (
              <div className="flex items-center gap-2">
                ...
                >
                  {currentStatus === 'active' ? (
                    ...
                    <XIcon className="mr-1 h-3 w-3 stroke-red-500 dark:stroke-red-400" />
                  ) : (
                    <HourglassIcon className="mr-1 h-3 w-3 stroke-yellow-500 dark:stroke-yellow-400" />
                  )}
                  {currentStatus}
                </Badge>
              </div>
            ...
            <div className="flex items-center gap-2">
              <Select
                ...
                onValueChange={(value) => {
                  onStatusUpdate({
                    directoryId,
                    status: value as 'active' | 'pending' | 'rejected',
                  });
                }}
                disabled={isUpdating}
              >
    ...
    [onStatusUpdate, updatingDirectoryId]
  );
