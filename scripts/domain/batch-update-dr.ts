...
import { domain, domainDrHistory } from '../../src/db/schema.js';
import { fetchDomainRating } from '../../src/lib/ahrefs.js';

// Load environment variables
// Supports ENV_FILE env var or defaults to .env
// For production: ENV_FILE=.env.production tsx scripts/dr/batch-update-dr.ts
...
// Progress file path
const PROGRESS_FILE = path.join(__dirname, 'batch-update-dr-progress.json');
 ...
 */
async function batchUpdateDomainRating() {
  try {
    // Parse command line arguments
    ...
    // Check API key
    if (!process.env.RAPIDAPI_AHREFS_KEY) {
      console.error('❌ RAPIDAPI_AHREFS_KEY is not configured');
      console.error('Please set RAPIDAPI_AHREFS_KEY in your .env file');
      process.exit(1);
    }
    ...
    for (let i = 0; i < domainsToProcess.length; i++) {
      ...
      try {
        console.log(`${progress} Fetching DR for ${domainItem.url}...`);
        ...
        console.log(
          `✅ ${progress} Updated DR for ${domainItem.url}: DR=${roundedDomainRating ?? 'N/A'}, AhRank=${domainMetrics.ahRank ?? 'N/A'}`
        );
        ...
        console.error(
          `❌ ${progress} Error updating domain rating for ${domainItem.url}:`,
          errorMessage
        );
...
// Run batch update
batchUpdateDomainRating().catch((error) => {
